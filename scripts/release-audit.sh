...
if rg -g '!scripts/release-audit.sh' '\.agents|\]\([^)]*(GOAL|PLAN)\.md' README.md README_zh_CN.md docs examples/README.md scripts .github >/tmp/nox-release-audit-links.$$ 2>/dev/null; then
...
if rg -q 'withdrawn|deprecated|hotfix|下游升级路径|git tag|GitHub Release' docs/release-checklist.md; then
    pass "release checklist includes rollback/tag/release terms"
else
    fail "release checklist is missing rollback/tag/release terms"
fi

if rg -q 'production release|checkpoint|release candidate|Breaking changes' README.md CHANGELOG.md docs/release-checklist.md; then
