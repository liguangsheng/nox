...
`env_list` 返回当前进程环境的字典。这些环境能力都需要 `environment` 权限。`sleep_ms`
阻塞当前线程，参数必须是非负毫秒。
...
这类顶层调用失败时，会释放本次调用中新建且仍 pending 的 task；失败前已经由更早调用创建的
pending task 仍保留，直到宿主继续 poll、cancel 或丢弃整个 `Runtime`。
...
- 文件系统标准库当前不做沙箱：脚本可以读写任意宿主可访问的路径。需要范围
  限制的宿主应在外部包装这些 host function。
