...
| `generic.constraint-unknown` | 稳定 | 在 `<T: Marker>` 中使用未知的约束 marker 名字。当前 marker 集合在 typecheck 内置，不允许用户扩展。 | `language_tests::constraint_unknown_marker_is_rejected_with_stable_code` |
| `parse.reserved-keyword` | 稳定 | 源码使用 `try` / `catch` / `panic` / `defer` / `finally` 作为标识符。这些词按 ADR 0021 暂留作未来异常机制评估的保留字。 | `language_tests::reserved_keywords_for_future_exceptions_are_rejected` |
...
| `runtime.task-pending-cap` | 稳定 | `task_sleep_ms` 将让当前 `Runtime` 的 pending sleep task 数超过 `RuntimePermissions::async_task_max_pending` 配置上限。默认上限为 1024。 | `tests::async_task_sleep_respects_pending_task_cap` |
