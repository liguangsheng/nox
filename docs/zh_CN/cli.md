...
nox project check
nox fetch [--offline] [--check|--locked] [--cache-dir <dir>]
...
source/pin drift；它不会因为缺 lock/cache 而联网。声明了 `[codegen]` 时，它只读校验
每个 artifact 的 generated `.nox` 文件存在，并把 generator/template/input hash/command
以及 source-map 路径/hash 元数据写入 JSON；声明的 source-map 文件会校验存在性，但不会解释内容。
它不会执行生成器，也不会把诊断或 rename/definition 穿透到模板。默认输出是
...
GitHub/git dependency 声明、lockfile 状态、codegen 元数据、模块图、三个子步骤的退出码以及
被捕获的 stdout/stderr，便于 CI 判断项目发现边界、能力边界和失败步骤。任一子步骤、lockfile
校验或 codegen 元数据校验失败时，命令返回非零；用法错误或缺少 manifest 返回 `2`。
...
cargo run -p nox -- fetch --check
cargo run -p nox -- fetch --locked
...
`--check` 和 `--locked` 是只读模式：它们解析 manifest、解析 dependency pin、计算当前 cache
内容的 hash，并验证现有 `nox.lock` 是否仍匹配，但不会写入 lockfile。`--check` 适合 CI 中
确认 lockfile 是否需要更新；`--locked` 用于要求当前 lockfile/cache 已经可用。二者都可以和
`--offline` 组合，此时命令只消费已有 cache；cache miss、corrupt cache、缺少 lockfile 或
lockfile drift 都会返回非零。
...
```json
{
  ...
  "schema_validation": {
    "ok": true,
    "manifest_sections": ["package", "entrypoints", "modules", "dependencies", "runtime", "codegen"],
  ...
  "codegen": {
    ...
    "artifacts": [
      {
        "name": "api",
        "generated": "/path/to/project/src/generated/api.nox",
        "exists": true,
        "generator": "tools/gen-api",
        "template": "/path/to/project/schemas/api.tpl",
        "input_hash": "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
        "source_map": "/path/to/project/src/generated/api.nox.map",
        "source_map_exists": true,
        "source_map_hash": "sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
...
源码签名；signature help 保留源码返回类型，同时标注调用侧返回 `task[T]`。泛型函数的
hover 和 signature help 会保留函数级 trait bound，例如
...
`textDocument/codeAction` 在 initialize 中声明 `quickfix`、`source.fixAll.nox` 和
`source.format.nox`。当前返回 `nox.check` 与 `nox.format` source action；如果已打开
文档包含可见 `TODO` marker，还会返回精确删除该 marker 的 quickfix edit。
...
`textDocument/semanticTokens/full` 对已打开文档返回词法级 semantic token stream；
legend 覆盖 namespace alias、type、function、variable、keyword、string、number 和
comment，并支持 declaration、readonly、async modifier。它不为 generated/codegen source
或 external dependency source map 建立额外映射。
如果打开文件匹配 manifest `[codegen]` artifact 的 `generated` 路径，hover 会追加只读
generated source 标注，包含 artifact 名称以及可选 generator/template/input hash/source map/
source map hash/command 元数据。该标注不会执行生成器、读取或解释 source map，也不会把
...
当前 LSP 继续作为集成式 `nox lsp` 子命令交付，不拆独立 LSP 二进制或单独 package。
它不支持跨文件 rename、generated/codegen source map、后台 watch mode 或 daemon 模式；
这些 request 不在 `initialize` capability 中声明。
