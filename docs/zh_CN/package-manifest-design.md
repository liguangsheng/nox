...
`v0.0.7` 起，上述 lockfile 形状是稳定化对象：`[lock].version = "1"`、dependency
section 名称、`source_kind` / `source`、`pin_kind` / `pin`、`resolved`、`content_hash`、
`cache_key` 和 `tool` 的含义不能静默改变。新增字段必须是旧 parser 可拒绝或旧 consumer 可忽略的
兼容扩展，并写入 CHANGELOG；删除字段、改变 hash 语义、改变 cache key 复现规则或让
`project check` 静默联网都视为 breaking change，需要迁移说明。
...
私有源和镜像边界保持保守：HTTPS / SSH / file git source 可以由宿主环境、凭据 helper 或本地镜像
提供访问，但 Nox 不保存凭据、不引入账号系统、不承诺中心撤回机制，也不把私有源解析结果写到
lockfile 之外的项目源码。私有 repo cookbook 和 mirror 建议可以后续补文档；在那之前，release gate
只承诺 pinned source、content hash、cache key、offline 失败和 no implicit network import 的行为。
