...
`0.0.4`。本仓库已经配置 `origin` remote；release 收口时必须同时核对本地 tag、远端 tag
...
1. [ ] 在 `Cargo.toml` 中把 `[workspace.package].version` 改成目标版本，例如
       `0.0.5`。运行 `cargo build` 让 `Cargo.lock` 更新。
2. [ ] 把 CHANGELOG 的"未发布"标题改为目标版本标题，例如 `## [0.0.5] — YYYY-MM-DD`，并在文件
...
7. [ ] **必做**：先跑 `scripts/local-dist-smoke.sh` 确认本地分发产物可用；然后跑
       `scripts/build-release-assets.sh`（无参数默认使用当前 Cargo 版本对应的 tag）。
       该脚本在隔离 worktree 上 release build，产出四个文件到 `/tmp/nox-release-assets-vX.Y.Z/`：
       - `nox-cli-vX.Y.Z-x86_64-unknown-linux-gnu.tar.gz`（`bin/nox` + `examples/`，不含 embed/）
       - `nox-cli-vX.Y.Z-x86_64-unknown-linux-gnu.sha256`
       - `nox-embed-vX.Y.Z-x86_64-unknown-linux-gnu.tar.gz`（`lib/libnox_core.so` + `include/nox_core.h` + embed C 示例 + 项目 README/CHANGELOG）
       - `nox-embed-vX.Y.Z-x86_64-unknown-linux-gnu.sha256`

       Inspect 内容后用 `gh release upload vX.Y.Z <四个文件>` 上传。每个 tarball 必须有对应
       `.sha256` 与之一并上传。当前仅承诺 `x86_64-unknown-linux-gnu`，其他目标三元组 best-effort（用
       `TARGET_TRIPLE=...` 环境变量覆盖默认值）。本步骤不能跳过——release 缺少二进制资产等于
       下游用户只能从源码构建。
...
- 暂缓项守护（第 13 项）：项目暂缓项清单（由内部 agent 计划维护）与上一 release tag 之间无静默 diff；
  ...
  incremental typecheck、tracing gc、package registry、lockfile）的未文档化命中。由 `P8.6` 在
  release-audit 中执行。
