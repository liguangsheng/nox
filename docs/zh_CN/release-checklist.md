...
- `[workspace.package].version` 仍是上一个已准备发布版本。
...
1. [ ] 在 `Cargo.toml` 中把 `[workspace.package].version` 改成目标版本。推荐运行
       `scripts/prepare-release-version.sh --check-only <version> YYYY-MM-DD` 先做只读锚点预检，再运行
       `scripts/prepare-release-version.sh <version> YYYY-MM-DD` 生成 release-prep diff；该脚本只改
...
2. [ ] 确认 CHANGELOG 的"未发布"标题已经变成目标版本标题，例如
       `## [<version>] — YYYY-MM-DD`，并在文件顶部重新开了新的"未发布"节，留给下一轮变更。
3. [ ] 创建一条单独的 release commit，例如 `Prepare vX.Y.Z release`。提交内容只包含
       版本号 bump、`Cargo.lock` 更新、CHANGELOG 标题切换，以及 README / docs 中当前生产版本
       口径同步。
4. [ ] 打 tag，例如 `git tag vX.Y.Z`。tag 名格式固定为 `vMAJOR.MINOR.PATCH`。
...
6. [ ] 在 GitHub Releases 中基于 tag 创建 release，用
       `NOX_RELEASE_VERSION=<version> scripts/release-notes.sh` 生成 release notes；
...
7. [ ] **必做**：先跑 `scripts/local-dist-smoke.sh` 确认本地分发产物可用；然后跑
       ...
       `.sha256` 与之一并上传。上传前还必须用同一个 `NOX_RELEASE_ASSET_DIR` 跑
       `scripts/release-asset-smoke.sh`；如果 release 资产后续被修复、镜像或重新下载，也对下载后的
       asset 目录再跑一次。该 smoke 会校验 `.sha256`、解包 tarball、对 host-compatible CLI
       asset 执行 `examples/hello.nox`，并对 host-compatible embed SDK 编译 C ABI smoke。
       当前完整 SDK 仅承诺 `x86_64-unknown-linux-gnu`；其他完整
       ...
       资产等于下游用户只能从源码构建。
       CI 中的 `Platform CLI smoke` matrix 会在 Linux、macOS 和 Windows runner 上运行
       `scripts/cli-smoke.sh`，覆盖 `nox --version`、`nox run examples/hello.nox` 和
       `nox check examples/hello.nox`。这只是 CLI-only 证据：不新增 macOS / Windows release
       asset 必需项，不承诺这些平台的 embed SDK，也不能替代 target-specific C ABI smoke。
       `scripts/release-asset-manifest.sh --json` 会输出同一份必需资产清单，并附带机器可读的
       `kind`、`target`、`commitment` 和 `c_abi_smoke_required` 字段，供 evidence report
       和人工审计使用；默认文本输出仍是 upload / cutover 脚本的兼容输入。
