...
| `std/fs.nox` | `read_text_async` / `try_read_text_async` / `exists_async` / `is_file_async` / `is_dir_async` / `list_dir_async` / `write_text_async` / `read_binary_async` / `write_binary_async` / `canonicalize_async` | `filesystem` / `filesystem_write` | 对应同步 helper 的 `async fn` wrapper；调用侧返回 `task[T]`，await 后仍走同一 capability、allowlist、mock 和诊断 |
...
| `std/jsonl.nox` | `parse_lines(source: str) -> result[[json], str]` | 无 | 每行解析一个 JSON value；错误包含 1-based 行号 |
| `std/jsonl.nox` | `format_lines(values: [json]) -> str` | 无 | 用 `\n` 拼接每个 JSON value 的 `json.stringify` 结果，不追加结尾换行 |
| `std/csv.nox` | `parse_line(value: str) -> result[[str], str]` | 无 | n/a |
| `std/csv.nox` | `format_row(values: [str]) -> str` | 无 | n/a |
| `std/csv.nox` | `parse_rows(value: str) -> result[[[str]], str]` | 无 | 在内存中解析多行 CSV；错误包含行号；不是 streaming parser |
| `std/csv.nox` | `format_rows(values: [[str]]) -> str` | 无 | 在内存中格式化多行 CSV，以 `\n` 分隔 row |
| `std/tsv.nox` | `parse_line(value: str) -> result[[str], str]` | 无 | n/a |
| `std/tsv.nox` | `format_row(values: [str]) -> result[str, str]` | 无 | n/a |
| `std/tsv.nox` | `parse_rows(value: str) -> result[[[str]], str]` | 无 | 在内存中解析多行 TSV；错误包含行号；不是 streaming parser |
| `std/tsv.nox` | `format_rows(values: [[str]]) -> result[str, str]` | 无 | 在内存中格式化多行 TSV，字段含 tab 时返回 err |
| `std/hash.nox` | `sha256_hex(bytes: [int]) -> str` | 无 | 对 `[int]` 字节数组计算 SHA-256；元素必须在 `0..=255` |
...
| `std/array.nox` | `contains_equal<T: Eq>(values: [T], target: T) -> bool` | 无 | 使用 `std/array.nox` 导出的实验性 `Eq` trait；旧 `contains_value<T: Equatable>` 仍保留 |
| `std/array.nox` | `dedupe_equal<T: Eq>(values: [T]) -> [T]` | 无 | 使用 `Eq.equals` 去重；旧 `dedupe<T: Equatable>` 仍保留 |
...
| `std/http.nox` | `request(method: str, url: str, headers: map[str, str], body: str, timeout_ms: int) -> result[(int, map[str, str], str), str]` | `network` | 支持自定义请求 header，并返回 response headers；重复 response header 用 `", "` 折叠 |
...
| `std/http.nox` | `request_binary(method: str, url: str, headers: map[str, str], body: [int], timeout_ms: int) -> result[(int, map[str, str], [int]), str]` | `network` | 二进制 body 版本的通用 request |
| `std/http.nox` | `get_async` / `post_async` / `request_async` / `get_binary_async` / `post_binary_async` / `request_binary_async` | `network` | 对应同步 helper 的 `async fn` wrapper；调用侧返回 `task[result[...]]`，await 后仍走同一 network capability、mock、timeout 和响应上限 |
| `std/task.nox` | `sleep_ms(ms: int) -> int` | `async task` | 返回 task id |
| `std/task.nox` | `sleep(ms: int) -> task[null]` | `async task` | 返回可 `await` 的 sleep task |
...
`std/jsonl.nox` 是纯计算 JSON Lines helper。`parse_lines(source) -> result[[json],
str]` 每行解析一个 JSON value，失败时返回 `line 2: expected JSON value` 这类
1-based 行号错误；`format_lines(values) -> str` 用 `\n` 拼接 `json.stringify`
结果，不追加结尾换行。

`std/csv.nox` 和 `std/tsv.nox` 提供单行解析/格式化 helper，也提供
`parse_rows` / `format_rows` 处理内存中的多行文本；它们不是 streaming parser。CSV
支持双引号字段和 `""` 转义；TSV 以 tab 分隔并拒绝格式化包含 tab 的字段。坏输入返回
`result.err(message)`。

`std/hash.nox` 是纯计算 SHA-256 / HMAC-SHA256 helper。`sha256_text(value) -> str`
对 UTF-8 文本计算 digest，`sha256_hex(bytes) -> str` 对 `[int]` 字节数组计算 digest
（元素必须在 `0..=255`）。`hmac_sha256_text(key, value) -> str` 使用 UTF-8 key 对
UTF-8 文本计算 HMAC，`hmac_sha256_hex(key, bytes) -> str` 使用 `[int]` key 对 `[int]`
字节数组计算 HMAC。所有 helper 都返回小写 hex 字符串。
...
`std/fs.nox` 的 `_async` 后缀 helper（`read_text_async`、`try_read_text_async`、
`exists_async`、`is_file_async`、`is_dir_async`、`list_dir_async`、`write_text_async`、
`read_binary_async`、`write_binary_async`、`canonicalize_async`）只是对应同步 helper
的 `async fn` wrapper。调用侧返回 `task[T]`，可在 `async fn` 内 await；await 后仍走
相同的 filesystem / filesystem_write capability、allowlist、mock filesystem、返回值和
diagnostic。
...
HTTP 仅支持明文 `http://`。通用 `request` helper 会发送 runtime 默认的 `Host`、
`User-Agent`、`Accept` 和 `Connection: close`；自定义 header map 可追加其他 header。
自定义 `Content-Length` 会被拒绝，因为 Nox 会按 body 自动计算。返回的 response header
name 统一小写；重复 header 按出现顺序用 `", "` 合并。HTTPS/TLS、HTTP/2、chunked
transfer、streaming body、cookie jar、redirect policy 和 auth framework 仍不属于当前
runtime 表面。

`std/http.nox` 的 `_async` 后缀 helper（`get_async`、`post_async`、
`request_async`、`get_binary_async`、`post_binary_async`、`request_binary_async`）
只是现有阻塞 HTTP helper 的 `async fn` wrapper。它们让 `async fn` 内可以 `await`
HTTP 结果，但不引入 IO reactor、后台网络调度或隐式授权；await 后仍使用同一套
`network` capability、mock network、timeout、1 MiB 响应上限和 diagnostic。

`task_sleep_ms` 创建一个内部 sleep task 并返回非负 `int` 作为 task id。`task_sleep`
创建同一张 runtime task 表中的 sleep task，并返回可在 `async fn` 内 `await` 的
`task[null]`。二者都需要 `async task` capability。id 在同一个 `Runtime` 内单调递增，
不复用。当前 task 状态机：

| 状态 | 进入方式 | 后续行为 |
| --- | --- | --- |
| pending | `task_sleep_ms(ms)` 成功返回 id，或 `task_sleep(ms)` 成功返回 `task[null]` | `task_ready(id)` 在 deadline 前返回 `false`，任务保留；`await task_sleep(ms)` 会阻塞当前 VM 执行直到 deadline。 |
...
| rejected | `task_sleep_ms` / `task_sleep` 收到负数、未授权或超过 pending task 上限 | 不创建 id，不进入任务表。 |
...
`Runtime` 中 pending sleep task 数。达到上限时，`task_sleep_ms` 和 `task_sleep` 在创建
新任务前返回稳定 diagnostic code `runtime.task-pending-cap`。可信宿主如需自行做任务配额，
可以设为 `None`。

Rust embedding 宿主可以通过 `Runtime::spawn_sleep_task`、`Runtime::poll_async_task`
和 `Runtime::cancel_async_task` 驱动同一张单 runtime task 表。这些 API 复用
`async_tasks` 权限、pending 上限、unknown-id diagnostic 和顶层失败清理规则。当前阶段
C ABI 不暴露 runtime task handle。
...
Nox 不提供可捕获 runtime exception。权限不足、allowlist 越界、resource cap、host callback
panic、host callback 返回值类型错误、parser/typechecker 失败和 VM diagnostic 都不是
`result.err`；它们会终止当前 eval / test case / CLI 命令。普通可恢复失败应由 stdlib 或宿主
显式设计为 `result` / `option` 返回值。
