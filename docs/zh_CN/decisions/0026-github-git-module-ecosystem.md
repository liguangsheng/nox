...
`v0.0.7` 把 GitHub/git URL package 路线从“已实现功能”提升为稳定化对象，但仍不新增 registry、
publish、search、账号或中心撤回机制。

稳定边界如下：

- `nox.lock` schema version 仍为 `1`，dependency section 继续记录 source、pin、resolved commit、
  `content_hash`、`cache_key` 和生成工具版本。
- `rev` 必须是完整 commit hash；`tag` 必须解析到 `resolved` commit。branch/default-branch 不作为
  release 稳定 pin。
- `run` / `check` / `test` / LSP / `project check` 在 cache miss、cache corrupt、hash mismatch 或
  缺少 lockfile 时返回诊断，不联网下载。
- `nox fetch --check` / `--locked` 是 CI 中的只读验证入口；默认 `nox fetch` 才允许写 lockfile。
- 私有 repo 和镜像只通过宿主 git 凭据、SSH agent、HTTPS credential helper 或 file/git mirror 解决；
  Nox 不保存凭据、不提供账号系统、不承诺中心撤回或命名所有权。

release gate 继续用 lockfile guardrail、fetch offline/cache regression、external import cache/hash
regression 和 package guardrail 检查这些边界。后续如果要加入 JSON 输出、cache inspect/clean、
private repo cookbook 或 mirror cookbook，必须保持 no implicit network import 和 pinned-source 规则。
