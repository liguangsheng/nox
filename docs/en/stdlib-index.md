...
| Text & data | `std/jsonl.nox` | stable | parse_lines / format_lines for JSON Lines text; parse errors include 1-based line numbers |
| Text & data | `std/csv.nox` / `std/tsv.nox` | stable | single-row parse / format plus parse_rows / format_rows for in-memory multi-row text |
...
- Doc vs. registry consistency is guarded by tests: std modules returned by the
  runtime registry must appear in this index, and exported helpers must be
  mentioned here or in `docs/en/runtime.md`.
