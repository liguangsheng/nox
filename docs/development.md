...
该脚本使用 `examples/malformed/` 的固定 corpus，覆盖未闭合字符串、深嵌套表达式、
...
- 语法/词法错误放在 `examples/malformed/*.nox`，并加入脚本中 `check=1` / `fmt=1` 的列表。
- 可格式化的静态或 module resolver 错误放在 `examples/malformed/*.nox`，并加入
  `check=1` / `fmt=0` 的列表。
- manifest 错误放在 `examples/malformed/manifest-*` 目录，脚本从目录内运行无显式路径的
  `check` 和 `fmt --check`。
- LSP-only 输入放在 `examples/malformed/`，脚本生成 Content-Length frame 后喂给
  `nox lsp`。
